...
class elastic_map(object):
    ...
    def __init__(self, given_data, init=None, stretch=1.0, bend=1.0, termination='iter', termination_condition=10):
    ...
    def calc_Uh(self):
        ...
        for i in range(1, self.map_size - 1):
            ...
            if np.any(xt2 == 0):
                continue
            cost = cost + np.linalg.norm(xt1 - kv* np.abs(((1 + xt1**2)**(3/2))/xt2)**(1/3))
        return cost
